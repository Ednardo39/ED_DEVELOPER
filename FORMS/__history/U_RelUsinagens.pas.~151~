...
type
  Tfrm_RelUsinagens = class(Tfrm_PadraoRel)
    Timer1: TTimer;
    ...
    Q_pesq_padraoUSINAGEM: TStringField;
    Q_pesq_padraoDT_TRABALHO: TDateField;
    Q_pesq_padraoRETRAB_USI: TStringField;
    Q_pesq_padraoDT_RETRAB_USI: TDateField;
    lb_Resultado: TLabel;
    mk_inicio: TMaskEdit;
    lbl_EdtFinal: TLabel;
    mk_fim: TMaskEdit;
...
var
  frm_RelUsinagens: Tfrm_RelUsinagens;

implementation

{$R *.dfm}

procedure Tfrm_RelUsinagens.cb_chave_pesquisaChange(Sender: TObject);
begin
...
procedure Tfrm_RelUsinagens.DBGrid1DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  with DBGrid1 do
  begin
   if Odd(DataSource.DataSet.RecNo) then //clWebWheat
      Canvas.Brush.Color := clWebAntiqueWhite
   else
...
begin
 ...
 case cb_chave_pesquisa.ItemIndex of
   ...
   3:begin// pesquisa todas as peças USINADAS
   ...
   6:begin// pesquisa por periodo
       Q_pesq_padrao.SQL.Add('WHERE DT_TRABALHO BETWEEN :PINICIO AND :PFIM');
...
procedure Tfrm_RelUsinagens.Timer1Timer(Sender: TObject);
begin
//Insere dados no statusbar
Statusbar1.Panels[0].Text:=DateToStr(now);
Statusbar1.Panels[1].Text:=TimeToStr(now);
Statusbar1.Panels[2].Text:='SEJA BEM VINDO(A) AO SISTEMA';
Statusbar1.Panels[3].Text:='Relatório de USINAGENS:';
end;

procedure Tfrm_RelUsinagens.ed_nomeKeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
    begin
      pesq_Peca;
...
procedure Tfrm_RelUsinagens.bt_PesquisaClick(Sender: TObject);
begin
  pesq_peca;
end;
