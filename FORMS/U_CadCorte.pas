...
type
  Tfrm_CadCorte = class(Tfrm_PadraoCad)
    ...
    Q_PadraoCORTE: TStringField;
    Q_PadraoDT_TRAB_CORTE: TDateField;
    Q_PadraoRETRAB_CORTE: TStringField;
    Q_PadraoDT_RETRAB_CORTE: TDateField;
    Panel1: TPanel;
    Label3: TLabel;
    Label4: TLabel;
    Label5: TLabel;
...
procedure Tfrm_CadCorte.buscarCodOP;
begin
...
procedure Tfrm_CadCorte.buscarCodPeca;
begin
    Q_Padrao.Close; // Fecha conexao com banco
    Q_Padrao.SQL.Add('');// Limpa conexao com banco
    Q_Padrao.SQL.Clear;  // Limpa SQL
...
procedure Tfrm_CadCorte.edtCodigoOPExit(Sender: TObject);
begin
...
procedure Tfrm_CadCorte.edtCodigoOPKeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
  begin
    edt_DtTrab.Text := DateTostr(Now);
...
procedure Tfrm_CadCorte.edtCodPecaExit(Sender: TObject);
begin
  if edt_Trabalho.Text = '' then
   begin
    edt_DtTrab.Text := DateTostr(Now);
...
procedure Tfrm_CadCorte.edtCodPecaKeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
  begin
    edt_DtTrab.Text := DateTostr(Now);
...
procedure Tfrm_CadCorte.edt_RetrabKeyPress(Sender: TObject; var Key: Char);
begin
  edt_DtRETrab.Text := DateTostr(Now);
  if (key=#13) AND (edt_Retrab.Text = '') then
  begin
...
procedure Tfrm_CadCorte.edt_TrabalhoKeyPress(Sender: TObject; var Key: Char);
begin
  edt_DtTrab.Text := DateTostr(Now);
...
procedure Tfrm_CadCorte.salvar;
begin
...
procedure Tfrm_CadCorte.btn_SairClick(Sender: TObject);
begin
  Close;
end;

procedure Tfrm_CadCorte.btn_SalvarClick(Sender: TObject);
begin
  salvar;
end;

procedure Tfrm_CadCorte.Timer1Timer(Sender: TObject);
begin
  //Insere dados no statusbar
Statusbar1.Panels[0].Text:=DateToStr(now);
Statusbar1.Panels[1].Text:=TimeToStr(now);
Statusbar1.Panels[2].Text:='SEJA BEM VINDO(A) AO SISTEMA';
Statusbar1.Panels[3].Text:='CONTROLE DE CORTE';
end;

procedure Tfrm_CadCorte.btn_CodOPClick(Sender: TObject);
begin
  buscarCodOP;
end;

procedure Tfrm_CadCorte.btn_CodPrincipalClick(Sender: TObject);
begin
  buscarCodPeca;
end;
