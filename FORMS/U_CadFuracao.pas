...
begin
    ...
    begin
      if MessageDlg('Peça já passou pela FURAÇÃO! Deseja editar esse registro?',TMsgDlgType.mtWarning,[mbOk,mbNo],0)=mrOk then
      begin
...
procedure Tfrm_CadFuracao.edtCodigoOPExit(Sender: TObject);
begin
...
procedure Tfrm_CadFuracao.edtCodigoOPKeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
  begin
    edt_DtTrab.Text := DateTostr(Now);
...
procedure Tfrm_CadFuracao.edtCodPecaExit(Sender: TObject);
begin
  if edt_Trabalho.Text = '' then
   begin
    edt_DtTrab.Text := DateTostr(Now);
...
procedure Tfrm_CadFuracao.edtCodPecaKeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
  begin
    edt_DtTrab.Text := DateTostr(Now);
...
procedure Tfrm_CadFuracao.edt_RetrabKeyPress(Sender: TObject; var Key: Char);
begin
  edt_DtRETrab.Text := DateTostr(Now);
...
procedure Tfrm_CadFuracao.edt_TrabalhoKeyPress(Sender: TObject; var Key: Char);
begin
  edt_DtTrab.Text := DateTostr(Now);
...
Statusbar1.Panels[3].Text:='CONTROLE DE FURAÇÃO / CAIXARIA';
end;
